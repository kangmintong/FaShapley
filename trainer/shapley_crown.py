    ...
    for idx, layer in enumerate(layers[::-1]):
        ...
        with torch.no_grad():
            for i, data in enumerate(dataloader):
                ...
                permutations = [np.random.permutation(nnn) for _ in range(sv_samples)]
                if sv is None:
                ...
                for j in range(sv_samples):
                    ...
                    for jj in tqdm(permutations[j]):
                        ...
                        # print(cc+n)
                        # print(jj)
                        sv[cc:cc + n, jj] += (delta / sv_samples).squeeze().detach().cpu().numpy()
                        loss = new_loss
        ...
        # sv = np.mean(sv, 0)
        # torch.save(sv,f'sv4_{idx}')
        sv = torch.load(f'sv4_{idx}')
