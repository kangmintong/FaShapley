...
def mixtrain(model, device, val_loader, criterion, args, writer, epoch=0):
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(val_loader):
            ...
            # file = 'tmp_mixtrain_2.txt'
            # with open(file, 'a') as f:
            #     f.write(f'epoch {epoch}: clean acc: {top1.avg} robust acc: {sym_top1.avg}')
        ...
        progress.display(i)  # print final results
        print(f'epoch {epoch}: clean acc: {top1.avg} robust acc: {sym_top1.avg}')
...
def smooth(model, device, val_loader, criterion, args, writer, epoch=0):
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(val_loader):
            ...
            output_clean = F.softmax(model(images), -1)
            acc1_clean, acc5 = accuracy(output_clean, target, topk=(1, 5))
            accs_clean.append(acc1_clean[0].item())
            ...
            accs_robust.append(acc1[0].item())
        ...
        # progress.display(i)  # print final results
        print(f'clean acc: {np.mean(np.array(accs_clean))}')
        print(f'robust acc: {np.mean(np.array(accs_robust))}')
