...
class Interval_Bound(nn.Module):
	...
	def forward(self, X, y):
		...
		if self.worst_case:
			...
			# print(type(y))
			# # 5 * 1 * 10 - 1 * 10 * 10
			# a = torch.eye(out_features).type_as(X)[y].unsqueeze(1)
			# b = torch.eye(out_features).type_as(X).unsqueeze(0)
			# a = a.expand(-1, b.shape[1], -1)
			# print('ok1')
...
def naive_interval_analyze(net, epsilon, X, y,\
					use_cuda=True, parallel=False, norm="linf", reduction="true"):
	...
	if reduction=='none':
		iloss = nn.CrossEntropyLoss(reduction='none')(wc, y)
		ierr = 0
	else:
